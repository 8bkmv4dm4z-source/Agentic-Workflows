...
from execution.langgraph.run_audit import summarize_runs
...
class RunAuditTests(unittest.TestCase):
    def test_summarize_runs_success_and_failed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success["tool_history"] = [
                {"call": 1, "tool": "repeat_message", "args": {"message": "ok"}, "result": {"echo": "ok"}},
                {"call": 2, "tool": "sort_array", "args": {"items": [2, 1]}, "result": {"sorted": [1, 2]}},
            ]
    ...
    def test_summarize_runs_flags_fib_issue(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            rows = summarize_runs(checkpoint_db_path=checkpoint_db, memo_db_path=f"{temp_dir}/missing_memo.db")
