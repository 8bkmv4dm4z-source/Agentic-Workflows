...
gaps:
  - truth: "Tool calls from an Anthropic provider flow through ToolNode with handle_tool_errors=True — the XML/JSON envelope parser is retired for that path only"
    status: failed
    reason: "The 'tools' ToolNode is added to the compiled graph (builder.add_node) but has no incoming routing edges. tools_condition is imported but never wired as a conditional edge from any node to 'tools'. The XML/JSON envelope parser (_parse_all_actions_json) still executes unconditionally on ALL provider paths including Anthropic in _plan_next_action(). Tool calls cannot reach the ToolNode — it is a dead-end orphaned node."
    artifacts:
      - path: "src/agentic_workflows/orchestration/langgraph/graph.py"
        issue: "builder.add_node('tools', dedup_node) is present but no add_conditional_edges or add_edge routes to it. tools_condition is in the import block but never passed to add_conditional_edges. _parse_all_actions_json() runs unconditionally at line 613 regardless of P1_PROVIDER."
    missing:
      - "Wire tools_condition as a conditional edge from the 'plan' or agent node to the 'tools' node when P1_PROVIDER=anthropic"
      - "Add a return edge from 'tools' back to 'plan' (or equivalent agent loop-back)"
      - "Gate the _parse_all_actions_json() call in _plan_next_action() so it does NOT run on the Anthropic path"
      - "Update the test (test_tool_node_constructed_for_anthropic_path) to assert that the 'tools' node is actually REACHABLE via graph edges, not just present in the node set"
human_verification: []
---
...
| 2 | Tool calls for the Anthropic provider path flow through ToolNode with handle_tool_errors=True — XML/JSON envelope parser retired for that path | FAILED | ToolNode node added to graph but no routing edges connect to it; _parse_all_actions_json() runs unconditionally on all paths |
...
| `graph.py _compile_graph()` | `ToolNode(handle_tool_errors=True)` | P1_PROVIDER=anthropic gate | PARTIAL | Node constructed but no edges route to it |
| `graph.py plan node` | `tools_condition` routing to 'tools' | `add_conditional_edges` | NOT WIRED | tools_condition imported but never used in any add_conditional_edges call |
...
| LGUP-02 | 02-03-PLAN.md | ToolNode + tools_condition replacing XML/JSON envelope parser | BLOCKED | ToolNode node exists but is orphaned — no routing edges; XML/JSON parser still runs on all paths including Anthropic |
