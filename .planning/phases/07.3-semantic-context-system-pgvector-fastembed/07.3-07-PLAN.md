...
type: execute
wave: 5
depends_on: ["07.3-05", "07.3-06"]
files_modified:
  - tests/integration/test_mission_context_cascade.py
autonomous: true
requirements: [SCS-08, SCS-09, SCS-10]

must_haves:
  truths:
    - "Full cascade query cycle runs with real Postgres (pgvector in CI)"
    - "Two similar missions across runs result in mission 2 seeing mission 1 context"
    - "MissionContextStore.upsert + query_cascade round-trip works end-to-end"
    - "Tests are skipped in SQLite-only CI (no DATABASE_URL set)"
    - "Tests use pg_pool and clean_pg fixtures per project standard"
  artifacts:
    - path: "tests/integration/test_mission_context_cascade.py"
      provides: "Postgres integration tests for full cascade retrieval cycle (SCS-08, SCS-09, SCS-10)"
      contains: "test_cascade_full_cycle, test_cross_run_injection, test_smoke_two_runs"
  key_links:
    - from: "tests/integration/test_mission_context_cascade.py"
      to: "src/agentic_workflows/storage/mission_context_store.py"
      via: "Direct instantiation of MissionContextStore(pool=pg_pool)"
      pattern: "MissionContextStore.*pg_pool"
    - from: "tests/integration/test_mission_context_cascade.py"
      to: "src/agentic_workflows/context/embedding_provider.py"
      via: "MockEmbeddingProvider for deterministic 384-dim embeddings"
      pattern: "MockEmbeddingProvider"
    - from: "tests/integration/test_mission_context_cascade.py"
      to: "tests/conftest.py"
      via: "pg_pool and clean_pg fixtures (extended in Wave 0)"
      pattern: "pg_pool, clean_pg"
---

<objective>
Write the Postgres integration tests for the full 5-layer cascade cycle. Uses real pgvector queries against a live Postgres instance. Skipped automatically when DATABASE_URL is not set (standard pattern). Covers SCS-08, SCS-09, SCS-10 from VALIDATION.md.

Purpose: End-to-end validation that the storage layer works with real Postgres — exactly what CI will run in the postgres leg of the matrix.
Output: Integration test file with 3 test classes covering upsert→query round-trip, ContextManager injection, and two-run smoke test.
</objective>
...
<interfaces>
<!-- Mandatory test skip pattern (from RESEARCH.md Pattern 5) -->
```python
...
<!-- pg_pool and clean_pg fixture come from conftest.py (session-scoped pool, per-test TRUNCATE) -->
<!-- clean_pg now truncates mission_contexts and mission_artifacts (from Wave 0 extension) -->

<!-- MissionContextStore usage -->
```python
store = MissionContextStore(pool=pg_pool, embedding_provider=MockEmbeddingProvider())
store.upsert(run_id="r1", mission_id="m1", goal="...", status="completed",
             summary="...", tools_used=["write_file"], key_results={}, embedding=[...]*384)
results = store.query_cascade("similar goal", top_k=3)
```

<!-- ContextManager injection check -->
```python
cm = ContextManager(mission_context_store=store, embedding_provider=provider)
...
<task type="auto">
  <name>Task 1: Write Postgres integration tests for cascade store</name>
  <files>tests/integration/test_mission_context_cascade.py</files>
  <action>
    Create tests/integration/test_mission_context_cascade.py:
    ...
    class TestSmoketwOruns:
        ...
        def test_two_run_smoke(self, pg_pool, clean_pg):
            ...
            run1_embedding = _make_embedding(run1_goal)
  ...
  <verify>
    <automated>cd /home/nir/dev/agent_phase0 && python -m pytest tests/integration/test_mission_context_cascade.py --collect-only -q 2>&1 | tail -15</automated>
  </verify>
  <done>Integration test file exists and collects without errors. Tests are skipped when DATABASE_URL is not set (not failures). File structure follows mandatory skip pattern with pytest.importorskip + requires_postgres.</done>
</task>

</tasks>

<verification>
`pytest tests/integration/test_mission_context_cascade.py --collect-only -q` — shows tests collected with correct class structure.
`pytest tests/integration/test_mission_context_cascade.py -q 2>&1 | tail -5` — shows SKIPPED (not FAILED) when no DATABASE_URL.
`pytest tests/ -q --tb=no` — 823+ existing tests unaffected.
</verification>

<success_criteria>
- tests/integration/test_mission_context_cascade.py exists with 3 test classes (9+ test functions)
- Tests are skipped (not failed) when DATABASE_URL is not set
- pytest.importorskip("psycopg_pool") at module level
- requires_postgres marker used correctly per project standard
- Tests use pg_pool and clean_pg fixtures
- Full suite `pytest tests/ -q` still passes with 823+ tests
</success_criteria>

<output>
After completion, create `.planning/phases/07.3-semantic-context-system-pgvector-fastembed/07.3-07-SUMMARY.md`
</output>
