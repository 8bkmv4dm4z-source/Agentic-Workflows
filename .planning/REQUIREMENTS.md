...
- [x] **PROD-04**: `Dockerfile` and `docker-compose.yml` allow the full system (API + Postgres) to be started with a single `docker-compose up`
- [x] **PROD-05**: GitHub Actions CI pipeline runs `ruff check`, `mypy`, and `pytest` on every push, using `ScriptedProvider` (no live LLM calls in CI)
